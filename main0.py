...
def transform_ts(ts, n_dim=400):
    ...
    for i in range(0, sample_size, bucket_size):
        ...
        percentil_calc = np.percentile(ts_rerange,[25,50,75])
        perdis = percentil_calc[-1]-percentil_calc[0]
        ts_new.append(np.concatenate([np.asarray([max_val,min_val,mean,std,mad,mode,perdis,skew,kurtosis]),
                                      percentil_calc]))
...
submission['target'] = preds_test_new
